  ...
  return (
    <div className='flex flex-col justify-center items-center h-[calc(100%-64px)]'>
      <div className='flex flex-col w-full max-w-[700px] gap-12 mt-20'>
        ...
        {!!userOrders.length ? (
          userOrders.map(({ id, orderDate, status, totalPrice, positions, finalPrice }) => {
            const isDiscount = !(finalPrice === totalPrice);
            return (
              <div className='flex flex-col' key={id}>
                <h2 className='font-medium text-3xl mb-6'>
                  {translation.orderNumber}: {id}
                </h2>
                <div className='flex flex-col border p-6 shadow-xl rounded-xl'>
                  <div className='flex justify-between mb-7'>
                    <span className='text-xl'>
                      {translation.orderDate}: {format(new Date(orderDate), 'yyyy.MM.dd | HH:mm')}
                    </span>
                  ...
                  <div className='flex w-full justify-end items-center'>
                    {isDiscount && (
                      <span className='font-bold text-xl sm:text-xl whitespace-nowrap text-red-500 line-through mx-4'>
                        {totalPrice} zł
                      </span>
                    )}
                    <span className='text-2xl font-medium'>
                      {translation.totalAmount}: {finalPrice} zł
                    </span>
                  </div>
                </div>
              </div>
          ...
          <span className='flex justify-center text-4xl'>
            {translation.actuallyYouDontHaveAnyOrders}
          </span>
        )}
      </div>
    </div>
